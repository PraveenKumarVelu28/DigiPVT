...
@Component({
  selector: 'app-run-subject-load-validation',
  templateUrl: './run-subject-load-validation.component.html',
  styleUrls: ['./run-subject-load-validation.component.css']
})
export class RunSubjectLoadValidationComponent implements OnInit {
...
// if( this.cutofflist.length!=0){
  ...
  this.startdate = formatDate(myDate, format, locale);
                  ...
                  debugger;
                    ...
                    this.LOPDays = 0;
                    this.DigiofficeService.GetRunSubjectLoadValidation(this.ID1[i],  this.startdate, this.enddate).subscribe(
                      res => {
                        debugger;
                    ...
                    for (let i = 0; i < this.ID.length; i++) {
                      ...
                      this.EmployeeID = this.ID[i];
                          ...
                          debugger;
                            ...
                            this.LOPDays = 0;
                            this.DigiofficeService.GetRunSubjectLoadValidation(this.ID[i],  this.startdate, this.enddate).subscribe(
                              res => {
                                debugger;
                                this.StaffSalaryReports = res;
                                Swal.fire(
