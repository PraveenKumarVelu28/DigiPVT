...
@Component({
  selector: 'app-staff-shift-master',
  templateUrl: './staff-shift-master.component.html',
  styleUrls: ['./staff-shift-master.component.css']
})

export class StaffShiftMasterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let lastdayofyear = new Date('2023-12-31')
    this.maxdate = lastdayofyear.toISOString().split("T")[0];
    this.ShiftName = "";
    this.Staffid1 = "";
    ...
    this.DigiofficeService.UpdateStaffShiftDetails(entity)
      .subscribe({
        next: data => {
          ...
          location.href = "#/Employee/MyTeamWeeklyShift";
          this.loader = false;
          this.showPopup = 1;
          this.messageId = 10;
        }, error: (err) => {
    ...
    for (let i = 0; i <= this.selectedstaff.length; i++) {
      ...
      this.DigiofficeService.UpdateStaffShift(entity)
        .subscribe({
          next: data => {
            debugger
            location.href = "#/Employee/WeeklyShift"
            this.loader = false;
          }, error: (err) => {
