...
@Component({
  selector: 'app-upload-cola-values',
  templateUrl: './upload-cola-values.component.html',
  styleUrls: ['./upload-cola-values.component.css']
})
export class UploadColaValuesComponent implements OnInit {
  ...
  ngOnInit(): void {
    debugger
    this.GetUploadatcolavalidation();
  ...
  public GetUploadatcolavalidation(){
    debugger
    this.DigiPVTService.GetUploadatcolavalidation().subscribe(data => {
      debugger
    ...
    if (this.exceldata == undefined) {
      ...
      for (this.i = 0; this.i < this.exceldata.length; this.i++) {
          ...
          var eb = { 
            ...
            'Colaamount' : this.exceldata[this.i].Colaamount,
            'Colaadjustment' : this.exceldata[this.i].Colaadjustment,
            'Oldcolamount' : this.exceldata[this.i].Colaamount

          }
          this.DigiPVTService.InsertUploadatcolavalidation(eb).subscribe({
            next: data => {
            debugger
            this.StaffID=data;
