...
@Component({
  selector: 'app-upload-allowance',
  templateUrl: './upload-allowance.component.html',
  styleUrls: ['./upload-allowance.component.css']
})
export class UploadAllowanceComponent implements OnInit {
  ...
  public GetAllowancesvalidation(){
    debugger
    this.DigiPVTService.GetAllowancesvalidation().subscribe(data => {
      debugger
      ...
      for (this.i = 0; this.i < this.exceldata.length; this.i++) {
          ...
          var eb = { 
            ...
            'Allowancename' : this.exceldata[this.i].AllowanceName,
             'numberofunits' : this.exceldata[this.i].NoofUnits,
            'Date' : this.Paydate
          }
          this.DigiPVTService.InsertAllowancevalidation(eb).subscribe({
            next: data => {
            debugger
            this.StaffID=data;
  ...
  delete(ID : any){
    ...
    Swal.fire({
      ...
      if (result.value == true) {
        this.DigiPVTService.DeleteAllowancesvalidation(ID)
          .subscribe({
            next: (data: any) => {
              debugger
  ...
  fileName = 'Upload Allowance Bonus DetailsReports.xlsx';
  exportexcel(): void {
