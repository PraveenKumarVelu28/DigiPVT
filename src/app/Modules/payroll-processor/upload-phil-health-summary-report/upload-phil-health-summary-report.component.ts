...
@Component({
  selector: 'app-upload-phil-health-summary-report',
  templateUrl: './upload-phil-health-summary-report.component.html',
  styleUrls: ['./upload-phil-health-summary-report.component.css']
})
export class UploadPhilHealthSummaryReportComponent implements OnInit {
  ...
  filetype: any;
  file: any;
  fileName = 'PhilHealth Summary Reports.xlsx';
  i:any;
  startdate:any;
  Attachment:any;
  ...
  ngOnInit(): void {
    debugger
    this.GetUploadPhilHealthSummaryReport();
  ...
  public GetUploadPhilHealthSummaryReport(){
    debugger
    this.DigiPVTService.GetPhilHealthValidation().subscribe(data => {
      debugger
      this.philhealthlist = data;
      console.log("philhealthlist", this.philhealthlist);
    });
  }
    ...
    if (this.exceldata == undefined) {
      ...
      for (this.i = 0; this.i < this.exceldata.length; this.i++) {
          ...
          var eb = { 
            ...
            'CollegeTeachingLoadsSemiTwo' : this.exceldata[this.i].CollegeTeachingLoadsSemiTwo,
            'SystemGenerated_PHIC_Total' : this.exceldata[this.i].SystemGenerated_PHIC_Total,
            'SystemGenerated_PHIC_EE' : this.exceldata[this.i].SystemGenerated_PHIC_EE,
            'SystemGenerated_PHIC_ER' : this.exceldata[this.i].SystemGenerated_PHIC_ER,
            'PayDate' : this.Paydate == null ? '1990-01-01 00:00:00.000' : this.Paydate

          }
          this.DigiPVTService.InsertPhilHealthValidation(eb).subscribe({
            next: data => {
            debugger
            this.StaffID=data;
              Swal.fire('Saved Successfully')
            ...
            Swal.fire('Issue in Inserting Philhealth Validation');
            // Insert error in Db Here//         
          }
        }
        )
