...
@Component({
  selector: 'app-validated-subject-load-details',
  templateUrl: './validated-subject-load-details.component.html',
  styleUrls: ['./validated-subject-load-details.component.css']
})
export class ValidatedSubjectLoadDetailsComponent implements OnInit {
  ...
  public GetValidatedSubjectLoadValues() {
    ...
    this.DigiofficeService.GetValidatedSubjectLoadValues()
      .subscribe({
        next: data => {
          debugger
          this.timedetails = data.filter(x=>x.regularBit==1);
  ...
  delete(ID : any){
    ...
    Swal.fire({
      ...
      if (result.value == true) {
        this.DigiofficeService.DeleteValidatedSubjectLoadValues(ID)
          .subscribe({
            next: data => {
              debugger
