...
import { AttendanceCorrectionFormComponent } from '../../employee/Attendance/attendance-correction-form/attendance-correction-form.component';
...
export class ManagerDashboardComponent implements OnInit {
  ...
  constructor(public router: Router, private matDialog: MatDialog, private datePipe: DatePipe, public DigiofficeService: DigiofficecorehrService, private http: HttpClient) { }
  username: any;
  email: any;
  month: any;
  day: any;
    ...
    this.DigiofficeService.GetCurrentPhTime(this.staffID, this.todaydate, this.todaydate)
      .subscribe({
        next: data => {
          ...
          this.abssentdays = temp[0].abssentdays;
          this.presentdays = temp[0].presentdays;
          this.approvedleavecount = temp[0].approvedleavecount;
          this.pendingleavecount = temp[0].pendingleavecount;
    ...
    this.DigiofficeService.GetAnnouncementsByBuildingID(56)
      .subscribe({
        next: data => {
          ...
          this.annoucemntattachment = this.newannnounecemnetlist[0].attachment;
          this.annname = this.newannnounecemnetlist[0].name
          this.anndateTime = this.newannnounecemnetlist[0].dateTime
        }, error: (err) => {
          // Swal.fire('Issue in Getting Announcements');
          // Insert error in Db Here//
          var obj = {
  ...
  gotomyPayslip() {
    this.router.navigate(['/HR/payslipview']);
  }
